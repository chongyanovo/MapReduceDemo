...
public class PartitionCount {
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        FileInputFormat.setInputPaths(job, new Path("H:\\MapReducer\\FlowPartitionCount\\in"));
        ...
        FileOutputFormat.setOutputPath(job, new Path("H:\\MapReducer\\FlowPartitionCount\\out"));
...
class MyPartition extends Partitioner<Text, NullWritable> {
    ...
    public int getPartition(Text text, NullWritable nullWritable, int i) {
        ...
        if (split[0].startsWith("135")) {
            return 0;
        } else if (split[0].startsWith("136")) {
            return 1;
        } else if (split[0].startsWith("137")) {
            return 2;
        } else {
            return 3;
        }
